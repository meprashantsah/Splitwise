// // src/components/GroupBalances.jsx
...
//       <h2 className="text-xl font-bold mb-2">Group Balances</h2>
//       <input
//         type="number"
...
// src/components/GroupBalances.jsx
import React, { useState } from 'react';
import { getGroupBalances } from '../api'; // Assuming this API call exists
...
const GroupBalances = () => {
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      const data = await getGroupBalances(parseInt(groupId.trim())); // Ensure ID is parsed as integer
      setBalances(data);
    } catch (err) {
      setError(err.message || 'An unexpected error occurred while fetching group balances.');
    } finally {
  ...
  return (
    ...
    <div className="bg-white p-6 rounded-lg shadow-md mb-6">
      ...
      <div className="space-y-4 mb-6"> {/* Added spacing around input and button */}
        ...
        >
          {isLoading ? 'Fetching Balances...' : 'View Group Balances'}
        </button>
      </div>
      ...
      {balances.length > 0 && (
        <div className="mt-6">
          ...
          <ul className="space-y-3"> {/* Increased space between list items */}
            {balances.map((item) => (
              <li
                key={`${item.owed_by.id}-${item.owed_to.id}`} // Unique key for each balance item
              ...
              >
                <p className="text-base text-gray-800">
                  ...
                  <span className="font-medium">{item.owed_to.name}</span>
                </p>
                <p className="text-lg font-bold">
                  <span className="text-red-600">₹{item.amount.toFixed(2)}</span> {/* Highlight amount in red for debts */}
                </p>
              </li>
            ))}
          </ul>
      ...
      {!isLoading && !error && balances.length === 0 && groupId.trim() && (
          <div className="mt-6 p-4 rounded-md bg-blue-50 border border-blue-200 text-blue-800">
            <p className="font-medium">No outstanding balances found for Group ID: <span className="font-mono">{groupId}</span>. Everyone is settled!</p>
          </div>
      )}
    </div>
  );
